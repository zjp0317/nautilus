...
void _beginthread (VoidFunction x, int tmp, void * z)
{
    pthread_t pt;
    pthread_attr_t pa;
    //pthread_attr_init (&pa);

#if 0//1//defined(__SVR4)
...
char **          blkp =NULL;// new char *[MAX_BLOCKS] ;
int *           blksize = NULL;//new int[MAX_BLOCKS] ;
long            seqlock=0 ;
struct lran2_st rgen ;
...
// zjp
    blkp = (char**)malloc(sizeof(char*) * (num_chunks*chperthread )) ;
    blksize = (int*)malloc(sizeof(int) * ((num_chunks*chperthread )) ;
...
{
    thread_data *de_area = malloc(max_threads * sizeof(thread_data)) ;
...
static struct shell_cmd_impl larson_impl = {
    .cmd      = "larson",
    .help_str = "larson", 
    .handler  = handle_larson,
};
nk_register_shell_cmd(larson_impl);
