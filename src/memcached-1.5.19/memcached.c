...
#ifndef IOV_MAX
#if defined(__FreeBSD__) || defined(__APPLE__) || defined(__GNU__) || defined(__Nautilus__)
...
#define NAUTILUS_MEMCACHED_CMD_TOKENS   128
#define NAUTILUS_MEMCACHED_TOKEN_SIZE   64
#define NAUTILUS_MEMCACHED_STRING   "memcached"
static int
handle_memcached(char * buf, void * priv) {
    char * argv[NAUTILUS_MEMCACHED_CMD_TOKENS];
    char * tmp  = NULL;
    int count   = 0;
    int st, i;

    argv[count++] = malloc(sizeof(NAUTILUS_MEMCACHED_STRING));
    
    tmp = strtok(buf, " ");
    while (tmp) {
        argv[count] = malloc(NAUTILUS_MEMCACHED_TOKEN_SIZE);
        memset(argv[count], 0, NAUTILUS_MEMCACHED_TOKEN_SIZE);
        strncpy(argv[count], tmp, NAUTILUS_MEMCACHED_TOKEN_SIZE);
        tmp = strtok(NULL, " ");
        count++;
    }
    for (i = 0; i < count; i++) {
